...
- **AI:** Codex Sonnet + Haiku via `@anthropic-ai/sdk` — smart model selection, 18 intents, conversation history, vision API
...
| `src/ai.js` | Codex AI intent classifier (18 intents), smart model selection (Haiku/Sonnet), adaptive history, rate limiting, conversation context |
...
| `src/analyze.js` | Document/image analysis using Codex Vision API (images + PDFs) |
...
17. `research` — multi-source web research via DuckDuckGo + Codex synthesis
...
- **Photo/document flow:** Photos stored as pending → user chooses: set reminder time, analyze, or save. Documents auto-analyzed (PDFs via Codex Vision).
- **AI-first routing:** All messages go through Codex AI before chrono-node fallback parser.
...
- **Haiku model:** Uses `Codex-haiku-4-5-20251001`. If API changes model names, update in `src/ai.js`.
